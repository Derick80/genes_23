...
type Props = {
  ...
  handleListCriteriaUsed: (value: string, evidenceType: string, weight:number) => void;
};
  ...
  return (
    <div className="flex flex-col">
      {Object.entries(data).map((key, index) => {
        const dataagain = options.filter((item) => item.evidenceType === key[0]);       

        
        return (
          <>
            <Label className="text-white">{key[0]}</Label>
            ...
            >
              <Select.Root
                name={key[0]}
                onValueChange={(value) => [
                  handleListCriteriaUsed(value, key[0], dataagain.filter((item:{
                    id: string;
                    evidenceType: string;
                    label: string;
                    definition: string;
